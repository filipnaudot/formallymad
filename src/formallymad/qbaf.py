...
class QBAFResolver:
    ...
    def resolve(self, tool_proposals: list[tuple[WorkerAgent, str, str]], *, visualize: bool = False) -> tuple[str, list[tuple[str, float]]]:
        ...
        qbaf = QBAFramework(args, initial_strengths, *self._build_relations(mapping), semantics=self._semantics)
        ...
        ctrbs = [(agent.id(), cast(float, determine_gradient_ctrb(max_tool, {agent.id()}, qbaf))) for agent in self._agents]
    ...
    def _build_relations(self, mapping: dict[str, str]) -> tuple[list[tuple[str, str]], list[tuple[str, str]]]:
        ...
        for i, agent in enumerate(self._agents):
            ...
            for prior in reversed(self._agents[:i]):
